...
const initialState = (id, blank = false) => {
...
export default class WeaponSetList {
  ...
  initialize(id, blank = false) {
    return this.createOrUpdate(id, initialState(id, blank));
  }
    ...
    switch (prefix) {
      ...
      case "a":
        for (const v of val.split(",")) {
          ...
          set.augmentEquipments = Object.assign({}, set.augmentEquipments, {
            [count]: weaponData.augmentEquipmentsFind(id)
          });
        }
        break;
      case "s":
        for (const v of val.split(",")) {
          ...
          set.skills = Object.assign({}, set.skills, {
            [skill.typeId]: skill
          });
        }
        break;
      case "i":
        for (const v of val.split(",")) {
          ...
          set.items = Object.assign({}, set.items, {
            [item.typeId]: item
          });
        }
        break;
